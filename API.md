...
| `generated_keyframes` | int | No | `0` | Opt-in interior slots. Must be a JSON integer in `[0, 8]`; booleans, floats, and `null` are rejected. When positive, after `load_denoise_input` the adapter appends one `VideoGeneratedKeyframeSlots` item to `stage_1_conditionings`. Positions match `evenly_spaced_keyframe_positions`: `torch.linspace(0, num_frames - 1, count + 2)` rounded, endpoints excluded. Existing stage-1 items, seed, images, audio, and prompt tensors are unchanged. Stage 2 still refines the upscaled stage-1 latent, matching the official two-stage path. `0` or absent does not import the slot type and leaves the bundle untouched. Do not set this together with `generated_keyframe_positions`. |
| `generated_keyframe_positions` | list[int] | No | `[]` | Explicit interior pixel frames instead of even spacing. Must be a JSON list of at most 8 strictly increasing non-negative integers. Endpoints (`0` and the last frame) are rejected when the bundle frame count is known. Empty leaves the historical path. |
| `stage2_drop_end_image` | bool | No | `false` | Opt-in. When true, after load the adapter removes non-zero-frame images from the bundle `images` list so stage 2 does not rebuild the end anchor. Stage-1 conditionings are left untouched. Must be a JSON boolean. False or absent leaves images unchanged. Do not set this together with `end_image_strength`. |
| `end_image_strength` | float | No | absent | Opt-in. When set to a JSON number in `(0, 1]`, after load the adapter sets that strength on stage-1 end keyframe conditionings and on non-zero-frame images used to rebuild stage 2. Frame 0 is unchanged. Absent leaves the encoded strength untouched. Booleans are rejected. |
...
A background loop inside the arbiter process samples `nvidia-smi` utilization. High util is polled every 30s; when util is at or below 5% it polls every 5s. A kill requires **two** conditions at once, because an idle GPU on its own is not evidence of a hang — a healthy worker can spend minutes in CPU-only work or on LLMCache-served turns (photo-enhance's reference/climb phases do exactly this). If a **local** instance still has active jobs (not `loading`/`starting`, not remote) while GPU util stays continuously low for 180 seconds **and that worker has emitted no stdout/stderr for the same window**, the watchdog:
...
Each instance entry in the report carries `silent_for_seconds` (`-1` = the worker never emitted anything). While the GPU is idle but workers are still logging, the watchdog logs `GPU idle but workers still emitting progress; not killing` at most once a minute and keeps the idle clock running, so a worker that later goes quiet is killed one idle window after its last log line — not two.
