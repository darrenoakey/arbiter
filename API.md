...
| `stage2_drop_end_image` | bool | No | `false` | Opt-in. When true, after load the adapter removes non-zero-frame images from the bundle `images` list so stage 2 does not rebuild the end anchor. Stage-1 conditionings are left untouched. Must be a JSON boolean. False or absent leaves images unchanged. |
